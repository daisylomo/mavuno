...
from mavuno.auth.rate_limit import AuthRateLimitExceeded, DistributedAuthRateLimiter
...
async def register(
    ...
    await _enforce_rate_limit(request, "register", f"{payload.email or ''}|{payload.phone or ''}")
...
async def login(
    ...
    await _enforce_rate_limit(request, "login", payload.identifier)
...
async def refresh(
    ...
    await _enforce_rate_limit(request, "refresh", payload.refresh_token)
...
async def _enforce_rate_limit(request: Request, scope: str, subject: str) -> None:
    limiter: DistributedAuthRateLimiter = request.app.state.auth_rate_limiter
    client_address = request.client.host if request.client is not None else "unknown"
    try:
        await limiter.check(scope, client_address, subject)
